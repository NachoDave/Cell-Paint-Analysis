...
load('/data/CEAT/ImageDatasets/CellPaint_WTCis_EpiCompoundDoseResponse/RScripts_011122/SOM_Results_CisOnly.rdata')
load('/data/CEAT/ImageDatasets/CellPaint_WTCis_EpiCompoundDoseResponse/R_Results/WT_Cis_UT_Cis_Dox_IntegratedIntensity.rdata')
...
ggsave("/data/CEAT/ImageDatasets/CellPaint_WTCis_EpiCompoundDoseResponse/R_Results/SOM_Results_011223_CisOnly_2/SOM_Plots/Well_LevelPCs.png", width = 10, height = 5)

ggplot(WT_Cis_Cells_CmbNormPCADf[WT_Cis_Cells_CmbNormPCADf$cell_line == "WT", ], aes(x = PC1, y = PC2,  color = cell_line)) + 
    geom_point(size= 2, fill = "#00bfc4") + xlab(paste("PC1: Variance Explained = ", round(varExplained[1], 2), "%")) + theme_bw() + xlim(-60, 110) + ylim(-100, 70) +
...
ggsave("/data/CEAT/ImageDatasets/CellPaint_WTCis_EpiCompoundDoseResponse/R_Results/SOM_Results_011223_CisOnly_2/SOM_Plots/Well_LevelPCsWT.png", width = 10, height = 5)
...
ggsave("/data/CEAT/ImageDatasets/CellPaint_WTCis_EpiCompoundDoseResponse/R_Results/SOM_Results_011223_CisOnly_2/SOM_Plots/Well_LevelPCsCis.png", width = 10, height = 5)
# ggplot(WT_Cis_Cells_CmbNormPCADf, aes(x = PC1, y = PC2,  color = cell_line, shape = Treatment)) + 
#     geom_point(size= 2) + xlab(paste("PC1: Variance Explained = ", round(varExplained[1], 2), "%")) + theme_bw() +  xlim(-60, 110) + ylim(-100, 70) +
#                                ylab(paste("PC2: Variance Explained = ", round(varExplained[2], 1), "%")) + ggtitle("WT+Cis combined Normalization") + guides(colour = guide_legend(override.aes = list(size=10)))
...
ggsave("/data/CEAT/ImageDatasets/CellPaint_WTCis_EpiCompoundDoseResponse/R_Results/SOM_Results_011223_CisOnly_2/SOM_Plots/Well_LevelPCsWT_DOSE.png", width = 10, height = 5)

ggplot(WT_Cis_Cells_CmbNormPCADf[WT_Cis_Cells_CmbNormPCADf$cell_line == "Cis", ], aes(x = PC1, y = PC2,  color = Dose)) + 
    geom_point(size= 2) + xlab(paste("PC1: Variance Explained = ", round(varExplained[1], 2), "%")) + theme_bw() + xlim(-60, 110) + ylim(-100, 70) +
                               ylab(paste("PC2: Variance Explained = ", round(varExplained[2], 1), "%")) + ggtitle("Cis WT+Cis combined Normalization") + guides(colour = guide_legend(override.aes = list(size=10)))
ggsave("/data/CEAT/ImageDatasets/CellPaint_WTCis_EpiCompoundDoseResponse/R_Results/SOM_Results_011223_CisOnly_2/SOM_Plots/Well_LevelPCsCisDOse.png", width = 10, height = 5)
...
tsne_df <- data.frame(tsne_result$Y, cell_line = WT_Cis_Cells_CmbNorm$cell_line)  # Assuming "Category" is your category column

# Plot using ggplot
ggplot(tsne_df, aes(x = X1, y = X2, color = factor(cell_line))) +
  geom_point() +
...
## 
smpN <- 10000 # number of cells from each treatment/dose

## Combined normalization
WT_Cis_Cells_CmbNormUT <- WT_Cis_CisplatinDoxUntreatedNrm[which(grepl("untreated", WT_Cis_CisplatinDoxUntreatedNrm$Treatment)), ] %>% group_by(Dose, cell_line) %>% slice_sample(n = smpN)

WT_Cis_Cells_CmbNormPCAUT <- prcomp(as.matrix(WT_Cis_Cells_CmbNormUT[, datColsCmb]), scale. = F)

WT_Cis_Cells_CmbNormPCAUTDf <- cbind(WT_Cis_Cells_CmbNormUT[,1:13], 
                                          data.frame(PC1 = WT_Cis_Cells_CmbNormPCAUT$x[,1], 
                                                     PC2 = WT_Cis_Cells_CmbNormPCAUT$x[,2], 
                                                     PC3 = WT_Cis_Cells_CmbNormPCAUT$x[,3]))

WT_Cis_Cells_CmbNormPCAUTDf$Dose <- factor(as.numeric(WT_Cis_Cells_CmbNormPCAUTDf$Dose), levels = sort(unique(as.numeric(WT_Cis_Cells_CmbNormPCAUTDf$Dose))))

varExplained <- 100*WT_Cis_Cells_CmbNormPCAUT$sdev^2/sum(WT_Cis_Cells_CmbNormPCAUT$sdev^2)

ggplot(WT_Cis_Cells_CmbNormPCAUTDf, aes(x = PC1, y = PC2,  color = cell_line,)) + 
    geom_point(size= 0.5) + xlab(paste("PC1: Variance Explained = ", round(varExplained[1], 2), "%")) + theme_bw() +  xlim(-60, 110) + ylim(-100, 70) +
                               ylab(paste("PC2: Variance Explained = ", round(varExplained[2], 1), "%")) + ggtitle("WT+Cis combined Normalization") + guides(colour = guide_legend(override.aes = list(size=10)))

ggsave("/data/CEAT/ImageDatasets/CellPaint_WTCis_EpiCompoundDoseResponse/R_Results/SOM_Results_011223_CisOnly_2/SOM_Plots/Cell_Level_UntreatedPCs.png", width = 10, height = 5)
```
...
# segImPthAll <- paste0(meta_dat_all_cells$Segmentationpath,
#                       str_replace(meta_dat_all_cells$FilenameDNA, pattern = '.tif', paste0('_', meta_dat_all_cells$cell_line, '.jpeg')))
# segImPthAllExists <- file.exists(segImPthAll)
#
# segImPthSelection <- paste0(meta_dat_PC_selection$Segmentationpath,
#                       str_replace(meta_dat_PC_selection$FilenameDNA, pattern = '.tif', paste0('_', meta_dat_PC_selection$cell_line, '.jpeg')))
# segImPthSelectionExists <- file.exists(segImPthSelection)
#
# ## Keep nuclocs and metadata for the cells where segmentation images exist (comment for all cells)
# meta_dat_all_cells <- meta_dat_all_cells[segImPthAllExists,]
# meta_dat_PC_selection <- meta_dat_PC_selection[segImPthSelectionExists,]
#
# ## Nucleus locations
...
# nucLocs_all_cells <- nucLocs_all_cells[segImPthSelectionExists,]
# nucLocs_PC_selection <- nucLocs_PC_selection[segImPthSelectionExists,]
```
...
# makeImagePatches(data.frame(meta_dat_PC_selection), wt_cis_cmb_SOM_top_pcs36$unit.classif[segImPthSelectionExists], data.frame(nucLocs_PC_selection),
#                  "/data/CEAT/ImageDatasets/CellPaint_WTCis_EpiCompoundDoseResponse/R_Results/SOM_Results_011223_CisOnly_2//top_PCs/36_node/",
#                  prefix = "TopPCs_36", smpN = 24, setSeed = TRUE, seed = 47)
# 
# # Get the percentage of cells in each 
# pltCellsInClusters(WT_Cis_Cells_CmbNorm, wt_cis_cmb_SOM_top_pcs36$codes[[1]], wt_cis_cmb_SOM_top_pcs36$unit.classif, metColsCmb, "/data/CEAT/ImageDatasets/CellPaint_WTCis_EpiCompoundDoseResponse/R_Results/SOM_Results_011223_CisOnly_2/top_PCs/36_node/", k = 16)
# 
# #  Plot the cluster composite images
# hier_clst_top_pcs_36 <- cutree(hclust(dist(wt_cis_cmb_SOM_top_pcs_codes36, "canberra")), k = 16)
# 
# compPlotsFromClusters("/data/CEAT/ImageDatasets/CellPaint_WTCis_EpiCompoundDoseResponse/R_Results/SOM_Results_011223_CisOnly_2/top_PCs/36_node/",
...
## Top PC features ===================================================================================================================================================================== ##
# Make Image patches
#   makeImagePatches(data.frame(meta_dat_all_cells), wt_cis_cmb_SOM_top_pc_features36$unit.classif[segImPthAllExists], data.frame(nucLocs_all_cells),
#                  "/data/CEAT/ImageDatasets/CellPaint_WTCis_EpiCompoundDoseResponse/R_Results/SOM_Results_011223_CisOnly_2/top_features_from_PCs/36_node/",
#                  prefix = "TopPC_feats_36", smpN = 24, setSeed = TRUE, seed = 47)
# 
# # Get the percentage of cells in each 
# pltCellsInClusters(WT_Cis_CisplatinDoxUntreatedNrm[!(WT_Cis_CisplatinDoxUntreatedNrm$Treatment == "Doxorubicin"), ], wt_cis_cmb_SOM_top_pc_features36$codes[[1]], wt_cis_cmb_SOM_top_pc_features36$unit.classif, metColsCmb, "/data/CEAT/ImageDatasets/CellPaint_WTCis_EpiCompoundDoseResponse/R_Results/SOM_Results_011223_CisOnly_2/top_features_from_PCs/36_node/", k = 16)
# 
# #  Plot the cluster composite images
# hier_clst_top_pc_features_36 <- cutree(hclust(dist(wt_cis_cmb_SOM_top_pc_features36$codes[[1]], "canberra")), k = 16)
# 
# compPlotsFromClusters("/data/CEAT/ImageDatasets/CellPaint_WTCis_EpiCompoundDoseResponse/R_Results/SOM_Results_011223_CisOnly_2/top_features_from_PCs/36_node/",
...
# ## Area Shape ===================================================================================================================================================================== ##
# # Make Image patches
#   makeImagePatches(data.frame(meta_dat_all_cells), wt_cis_cmb_SOM_AreaShape36$unit.classif[segImPthAllExists], data.frame(nucLocs_all_cells),
#                  "/data/CEAT/ImageDatasets/CellPaint_WTCis_EpiCompoundDoseResponse/R_Results/SOM_Results_011223_CisOnly_2/areashape_features/36_node/",
#                  prefix = "AreaShape_36", smpN = 24, setSeed = TRUE, seed = 47)
# 
# # Get the percentage of cells in each 
...
# 
# compPlotsFromClusters("/data/CEAT/ImageDatasets/CellPaint_WTCis_EpiCompoundDoseResponse/R_Results/SOM_Results_011223_CisOnly_2/areashape_features/36_node/",
...
# ## All features ===================================================================================================================================================================== ##
# # Make Image patches
#   makeImagePatches(data.frame(meta_dat_all_cells), wt_cis_cmb_SOM_all_features36$unit.classif[segImPthAllExists], data.frame(nucLocs_all_cells),
#                  "/data/CEAT/ImageDatasets/CellPaint_WTCis_EpiCompoundDoseResponse/R_Results/SOM_Results_011223_CisOnly_2/all_features//36_node/",
#                  prefix = "All_features_36", smpN = 24, setSeed = TRUE, seed = 47)
# 
# # Get the percentage of cells in each 
# pltCellsInClusters(WT_Cis_CisplatinDoxUntreatedNrm[!(WT_Cis_CisplatinDoxUntreatedNrm$Treatment == "Doxorubicin"), ], wt_cis_cmb_SOM_all_features36$codes[[1]], wt_cis_cmb_SOM_all_features36$unit.classif, metColsCmb, "/data/CEAT/ImageDatasets/CellPaint_WTCis_EpiCompoundDoseResponse/R_Results/SOM_Results_011223_CisOnly_2/all_features//36_node/", k = 16)
# 
# #  Plot the cluster composite images
# hier_clst_all_features_36 <- cutree(hclust(dist(wt_cis_cmb_SOM_all_features36$codes[[1]], "canberra")), k = 16)
# 
# compPlotsFromClusters("/data/CEAT/ImageDatasets/CellPaint_WTCis_EpiCompoundDoseResponse/R_Results/SOM_Results_011223_CisOnly_2/all_features//36_node/",
...
## Top PCs ===================================================================================================================================================================== ##
# Make Image patches
makeImagePatches(data.frame(meta_dat_PC_selection), wt_cis_cmb_SOM_top_pcs25$unit.classif[segImPthSelectionExists], data.frame(nucLocs_PC_selection),
                 "/data/CEAT/ImageDatasets/CellPaint_WTCis_EpiCompoundDoseResponse/R_Results/SOM_Results_011223_CisOnly_2/top_PCs/25_node/",
                 prefix = "TopPCs_25", smpN = 24, setSeed = TRUE, seed = 47)

# Get the percentage of cells in each 
pltCellsInClusters(WT_Cis_Cells_CmbNorm, wt_cis_cmb_SOM_top_pcs25$codes[[1]], wt_cis_cmb_SOM_top_pcs25$unit.classif, metColsCmb, "/data/CEAT/ImageDatasets/CellPaint_WTCis_EpiCompoundDoseResponse/R_Results/SOM_Results_011223_CisOnly_2/top_PCs/25_node/", k = 16)

#  Plot the cluster composite images
hier_clst_top_pcs_25 <- cutree(hclust(dist(wt_cis_cmb_SOM_top_pcs_codes25, "canberra")), k = 16)

compPlotsFromClusters("/data/CEAT/ImageDatasets/CellPaint_WTCis_EpiCompoundDoseResponse/R_Results/SOM_Results_011223_CisOnly_2/top_PCs/25_node/",
...
## Top PC features ===================================================================================================================================================================== ##
# Make Image patches
#   makeImagePatches(data.frame(meta_dat_all_cells), wt_cis_cmb_SOM_top_pc_features25$unit.classif[segImPthAllExists], data.frame(nucLocs_all_cells),
#                  "/data/CEAT/ImageDatasets/CellPaint_WTCis_EpiCompoundDoseResponse/R_Results/SOM_Results_011223_CisOnly_2/top_features_from_PCs/25_node/",
#                  prefix = "TopPC_feats_25", smpN = 24, setSeed = TRUE, seed = 47)
# 
# # Get the percentage of cells in each 
# pltCellsInClusters(WT_Cis_CisplatinDoxUntreatedNrm[!(WT_Cis_CisplatinDoxUntreatedNrm$Treatment == "Doxorubicin"), ], wt_cis_cmb_SOM_top_pc_features25$codes[[1]], wt_cis_cmb_SOM_top_pc_features25$unit.classif, metColsCmb, "/data/CEAT/ImageDatasets/CellPaint_WTCis_EpiCompoundDoseResponse/R_Results/SOM_Results_011223_CisOnly_2/top_features_from_PCs/25_node/", k = 16)
# 
# #  Plot the cluster composite images
# hier_clst_top_pc_features_25 <- cutree(hclust(dist(wt_cis_cmb_SOM_top_pc_features25$codes[[1]], "canberra")), k = 16)
# 
# compPlotsFromClusters("/data/CEAT/ImageDatasets/CellPaint_WTCis_EpiCompoundDoseResponse/R_Results/SOM_Results_011223_CisOnly_2/top_features_from_PCs/25_node/",
...
# ## Area Shape ===================================================================================================================================================================== ##
# # Make Image patches
#   makeImagePatches(data.frame(meta_dat_all_cells), wt_cis_cmb_SOM_AreaShape25$unit.classif[segImPthAllExists], data.frame(nucLocs_all_cells),
#                  "/data/CEAT/ImageDatasets/CellPaint_WTCis_EpiCompoundDoseResponse/R_Results/SOM_Results_011223_CisOnly_2/areashape_features/25_node/",
#                  prefix = "AreaShape_25", smpN = 24, setSeed = TRUE, seed = 47)
# 
# # Get the percentage of cells in each 
...
# 
# compPlotsFromClusters("/data/CEAT/ImageDatasets/CellPaint_WTCis_EpiCompoundDoseResponse/R_Results/SOM_Results_011223_CisOnly_2/areashape_features/25_node/",
...
# ## All features ===================================================================================================================================================================== ##
# # Make Image patches
#   makeImagePatches(data.frame(meta_dat_all_cells), wt_cis_cmb_SOM_all_features25$unit.classif[segImPthAllExists], data.frame(nucLocs_all_cells),
#                  "/data/CEAT/ImageDatasets/CellPaint_WTCis_EpiCompoundDoseResponse/R_Results/SOM_Results_011223_CisOnly_2/all_features//25_node/",
#                  prefix = "All_features_25", smpN = 24, setSeed = TRUE, seed = 47)
# 
# # Get the percentage of cells in each 
# pltCellsInClusters(WT_Cis_CisplatinDoxUntreatedNrm[!(WT_Cis_CisplatinDoxUntreatedNrm$Treatment == "Doxorubicin"), ], wt_cis_cmb_SOM_all_features25$codes[[1]], wt_cis_cmb_SOM_all_features25$unit.classif, metColsCmb, "/data/CEAT/ImageDatasets/CellPaint_WTCis_EpiCompoundDoseResponse/R_Results/SOM_Results_011223_CisOnly_2/all_features//25_node/", k = 16)
# 
# #  Plot the cluster composite images
# hier_clst_all_features_25 <- cutree(hclust(dist(wt_cis_cmb_SOM_all_features25$codes[[1]], "canberra")), k = 16)
# 
# compPlotsFromClusters("/data/CEAT/ImageDatasets/CellPaint_WTCis_EpiCompoundDoseResponse/R_Results/SOM_Results_011223_CisOnly_2/all_features//25_node/",
...
## Top PCs ===================================================================================================================================================================== ##
# Make Image patches
s# makeImagePatches(data.frame(meta_dat_PC_selection), wt_cis_cmb_SOM_top_pcs16$unit.classif[segImPthSelectionExists], data.frame(nucLocs_PC_selection),
#                  "/data/CEAT/ImageDatasets/CellPaint_WTCis_EpiCompoundDoseResponse/R_Results/SOM_Results_011223_CisOnly_2/top_PCs/16_node/",
#                  prefix = "TopPCs_16", smpN = 24, setSeed = TRUE, seed = 47)
# 
# # Get the percentage of cells in each 
# pltCellsInClusters(WT_Cis_Cells_CmbNorm, wt_cis_cmb_SOM_top_pcs16$codes[[1]], wt_cis_cmb_SOM_top_pcs16$unit.classif, metColsCmb, "/data/CEAT/ImageDatasets/CellPaint_WTCis_EpiCompoundDoseResponse/R_Results/SOM_Results_011223_CisOnly_2/top_PCs/16_node/", k = 16)
# 
# #  Plot the cluster composite images
# hier_clst_top_pcs_16 <- cutree(hclust(dist(wt_cis_cmb_SOM_top_pcs_codes16, "canberra")), k = 16)
# 
# compPlotsFromClusters("/data/CEAT/ImageDatasets/CellPaint_WTCis_EpiCompoundDoseResponse/R_Results/SOM_Results_011223_CisOnly_2/top_PCs/16_node/",
...
# ## Top PC features ===================================================================================================================================================================== ##
# # Make Image patches
#   makeImagePatches(data.frame(meta_dat_all_cells), wt_cis_cmb_SOM_top_pc_features16$unit.classif[segImPthAllExists], data.frame(nucLocs_all_cells),
#                  "/data/CEAT/ImageDatasets/CellPaint_WTCis_EpiCompoundDoseResponse/R_Results/SOM_Results_011223_CisOnly_2/top_features_from_PCs/16_node/",
#                  prefix = "TopPC_feats_16", smpN = 24, setSeed = TRUE, seed = 47)
# 
# # Get the percentage of cells in each 
# pltCellsInClusters(WT_Cis_CisplatinDoxUntreatedNrm[!(WT_Cis_CisplatinDoxUntreatedNrm$Treatment == "Doxorubicin"), ], wt_cis_cmb_SOM_top_pc_features16$codes[[1]], wt_cis_cmb_SOM_top_pc_features16$unit.classif, metColsCmb, "/data/CEAT/ImageDatasets/CellPaint_WTCis_EpiCompoundDoseResponse/R_Results/SOM_Results_011223_CisOnly_2/top_features_from_PCs/16_node/", k = 16)
# 
# #  Plot the cluster composite images
# hier_clst_top_pc_features_16 <- cutree(hclust(dist(wt_cis_cmb_SOM_top_pc_features16$codes[[1]], "canberra")), k = 16)
# 
# compPlotsFromClusters("/data/CEAT/ImageDatasets/CellPaint_WTCis_EpiCompoundDoseResponse/R_Results/SOM_Results_011223_CisOnly_2/top_features_from_PCs/16_node/",
...
# ## Area Shape ===================================================================================================================================================================== ##
# # Make Image patches
#   makeImagePatches(data.frame(meta_dat_all_cells), wt_cis_cmb_SOM_AreaShape16$unit.classif[segImPthAllExists], data.frame(nucLocs_all_cells),
#                  "/data/CEAT/ImageDatasets/CellPaint_WTCis_EpiCompoundDoseResponse/R_Results/SOM_Results_011223_CisOnly_2/areashape_features/16_node/",
#                  prefix = "AreaShape_16", smpN = 24, setSeed = TRUE, seed = 47)
# 
# # Get the percentage of cells in each 
...
# 
# compPlotsFromClusters("/data/CEAT/ImageDatasets/CellPaint_WTCis_EpiCompoundDoseResponse/R_Results/SOM_Results_011223_CisOnly_2/areashape_features/16_node/",
...
# ## All features ===================================================================================================================================================================== ##
# # Make Image patches
#   makeImagePatches(data.frame(meta_dat_all_cells), wt_cis_cmb_SOM_all_features16$unit.classif[segImPthAllExists], data.frame(nucLocs_all_cells),
#                  "/data/CEAT/ImageDatasets/CellPaint_WTCis_EpiCompoundDoseResponse/R_Results/SOM_Results_011223_CisOnly_2/all_features//16_node/",
#                  prefix = "All_features_16", smpN = 24, setSeed = TRUE, seed = 47)
# 
# # Get the percentage of cells in each 
# pltCellsInClusters(WT_Cis_CisplatinDoxUntreatedNrm[!(WT_Cis_CisplatinDoxUntreatedNrm$Treatment == "Doxorubicin"), ], wt_cis_cmb_SOM_all_features16$codes[[1]], wt_cis_cmb_SOM_all_features16$unit.classif, metColsCmb, "/data/CEAT/ImageDatasets/CellPaint_WTCis_EpiCompoundDoseResponse/R_Results/SOM_Results_011223_CisOnly_2/all_features//16_node/", k = 16)
# 
# #  Plot the cluster composite images
# hier_clst_all_features_16 <- cutree(hclust(dist(wt_cis_cmb_SOM_all_features16$codes[[1]], "canberra")), k = 16)
# 
# compPlotsFromClusters("/data/CEAT/ImageDatasets/CellPaint_WTCis_EpiCompoundDoseResponse/R_Results/SOM_Results_011223_CisOnly_2/all_features//16_node/",
...
imageDendroHM("/data/CEAT/ImageDatasets/CellPaint_WTCis_EpiCompoundDoseResponse/R_Results/SOM_Results_011223_CisOnly_2//all_features/25_node/",
              ...
              "all_features_25_clst25_NoIsolate_cell_10im.png",  cut = 25, imN = 10, feature_type = "allFeatures", isolateCell = FALSE)
# # 
# imageDendroHM("/data/CEAT/ImageDatasets/CellPaint_WTCis_EpiCompoundDoseResponse/R_Results/SOM_Results_011223_CisOnly_2//all_features/25_node/",
#               wt_cis_cmb_SOM_all_features25,
#               WT_Cis_CisplatinDoxUntreatedNrm[!(WT_Cis_CisplatinDoxUntreatedNrm$Treatment == "Doxorubicin"), ],
#               metColsCmb, sWT_Cis_CisplatinDoxUntreatedIntegratedIntensity,
#               "All_features_25_",
#               "/data/CEAT/ImageDatasets/CellPaint_WTCis_EpiCompoundDoseResponse/R_Results/SOM_Results_011223_CisOnly_2/SOM_Plots2/",
#               "all_features_25_clst12.png",  cut = 12, imN = 5, feature_type = "allFeatures")
# #
# imageDendroHM("/data/CEAT/ImageDatasets/CellPaint_WTCis_EpiCompoundDoseResponse/R_Results/SOM_Results_011223_CisOnly_2//all_features/25_node/",
#               wt_cis_cmb_SOM_all_features25,
#               WT_Cis_CisplatinDoxUntreatedNrm[!(WT_Cis_CisplatinDoxUntreatedNrm$Treatment == "Doxorubicin"), ],
...
#               "All_features_25_",
#               "/data/CEAT/ImageDatasets/CellPaint_WTCis_EpiCompoundDoseResponse/R_Results/SOM_Results_011223_CisOnly_2/SOM_Plots2/",
#               "all_features_25_clst6.png",  cut = 6, imN = 5, feature_type = "allFeatures")
# 
# 
# ## AreaShape features
# imageDendroHM("/data/CEAT/ImageDatasets/CellPaint_WTCis_EpiCompoundDoseResponse/R_Results/SOM_Results_011223_CisOnly_2//areashape_features//25_node/",
#               wt_cis_cmb_SOM_AreaShape25,
#               WT_Cis_CisplatinDoxUntreatedNrm[!(WT_Cis_CisplatinDoxUntreatedNrm$Treatment == "Doxorubicin"), ],
#               metColsCmb, WT_Cis_CisplatinDoxUntreatedIntegratedIntensity,
#               "AreaShape_25_",
#               "/data/CEAT/ImageDatasets/CellPaint_WTCis_EpiCompoundDoseResponse/R_Results/SOM_Results_011223_CisOnly_2/SOM_Plots2/",
#               "AreaShape_25_clst25.png",  cut = 25, imN = 5, feature_type = "AreaShape")
...
#               "AreaShape_25_",
#               "/data/CEAT/ImageDatasets/CellPaint_WTCis_EpiCompoundDoseResponse/R_Results/SOM_Results_011223_CisOnly_2/SOM_Plots2/",
#               "AreaShape_25_clst12.png",  cut = 12, imN = 5, feature_type = "AreaShape")
...
#               "AreaShape_25_",
#               "/data/CEAT/ImageDatasets/CellPaint_WTCis_EpiCompoundDoseResponse/R_Results/SOM_Results_011223_CisOnly_2/SOM_Plots2/",
#               "AreaShape_25_clst6.png",  cut = 6, imN = 5, feature_type = "AreaShape")

## Top PC features
# imageDendroHM("/data/CEAT/ImageDatasets/CellPaint_WTCis_EpiCompoundDoseResponse/R_Results/SOM_Results_011223_CisOnly_2//top_features_from_PCs/25_node/",
#               wt_cis_cmb_SOM_top_pc_features25,
#               WT_Cis_CisplatinDoxUntreatedNrm[!(WT_Cis_CisplatinDoxUntreatedNrm$Treatment == "Doxorubicin"), ],
#               metColsCmb, WT_Cis_CisplatinDoxUntreatedIntegratedIntensity,
#               "TopPC_feats_25_",
#               "/data/CEAT/ImageDatasets/CellPaint_WTCis_EpiCompoundDoseResponse/R_Results/SOM_Results_011223_CisOnly_2/SOM_Plots2/",
#               "topPC_features_25_clst25.png",  cut = 25, imN = 5, feature_type = "allFeatures")
# 
# imageDendroHM("/data/CEAT/ImageDatasets/CellPaint_WTCis_EpiCompoundDoseResponse/R_Results/SOM_Results_011223_CisOnly_2//top_features_from_PCs/25_node/",
#               wt_cis_cmb_SOM_top_pc_features25,
#               WT_Cis_CisplatinDoxUntreatedNrm[!(WT_Cis_CisplatinDoxUntreatedNrm$Treatment == "Doxorubicin"), ],
...
#               "TopPC_feats_25_",
#               "/data/CEAT/ImageDatasets/CellPaint_WTCis_EpiCompoundDoseResponse/R_Results/SOM_Results_011223_CisOnly_2/SOM_Plots2/",
#               "topPC_features_25_clst12.png",  cut = 12, imN = 5, feature_type = "allFeatures")
# 
# imageDendroHM("/data/CEAT/ImageDatasets/CellPaint_WTCis_EpiCompoundDoseResponse/R_Results/SOM_Results_011223_CisOnly_2//top_features_from_PCs/25_node/",
#               wt_cis_cmb_SOM_top_pc_features25,
#               WT_Cis_CisplatinDoxUntreatedNrm[!(WT_Cis_CisplatinDoxUntreatedNrm$Treatment == "Doxorubicin"), ],
...
#               "TopPC_feats_25_",
#               "/data/CEAT/ImageDatasets/CellPaint_WTCis_EpiCompoundDoseResponse/R_Results/SOM_Results_011223_CisOnly_2/SOM_Plots2/",
#               "topPC_features_25_clst6.png",  cut = 6, imN = 5, feature_type = "allFeatures")

## Top PCs
# imageDendroHM("/data/CEAT/ImageDatasets/CellPaint_WTCis_EpiCompoundDoseResponse/R_Results/SOM_Results_011223_CisOnly_2//top_PCs//25_node/",
#               wt_cis_cmb_SOM_top_pcs25,
#               WT_Cis_Cmb_PCs,
#               metColsCmb, WT_Cis_CisplatinDoxUntreatedIntegratedIntensity,
#               "TopPCs_25_",
#               "/data/CEAT/ImageDatasets/CellPaint_WTCis_EpiCompoundDoseResponse/R_Results/SOM_Results_011223_CisOnly_2/SOM_Plots/TopPCs///",
#               "topPCs_25_clst25.png",  cut = 25, imN = 5, feature_type = "None")



# imageDendroHM("/data/CEAT/ImageDatasets/CellPaint_WTCis_EpiCompoundDoseResponse/R_Results/SOM_Results_011223_CisOnly_2//all_features/16_node/",
#               wt_cis_cmb_SOM_all_features16,
#               WT_Cis_CisplatinDoxUntreatedNrm[!(WT_Cis_CisplatinDoxUntreatedNrm$Treatment == "Doxorubicin"), ],
#               metColsCmb, WT_Cis_CisplatinDoxUntreatedIntegratedIntensity,
#               "All_features_16_",
#               "/data/CEAT/ImageDatasets/CellPaint_WTCis_EpiCompoundDoseResponse/R_Results/SOM_Results_011223_CisOnly_2/SOM_Plots2/",
#               "all_features_16_clst16.png",  cut = 25, imN = 5, feature_type = "allFeatures")

```
## Plots with cell percent per SOM class
```{r}
## Im
cell_25_all_feat_info <- WT_Cis_CisplatinDoxUntreatedNrm[!(WT_Cis_CisplatinDoxUntreatedNrm$Treatment == "Doxorubicin"), metColsCmb]
cell_25_all_feat_info$som_class <- wt_cis_cmb_SOM_all_features25$unit.classif

cell_25_all_feat_info_grps <- cell_25_all_feat_info %>% group_by(cell_line, Dose, som_class) %>% count() %>% summarise(nn = sum(n)) # %>% mutate(percentage_total = 100*nn/sum(nn)) 
cell_25_all_feat_info_grps$Dose <- factor(cell_25_all_feat_info_grps$Dose, c("0", "500", "2000", "5000", "10000", "20000", "40000", "80000"))
cell_25_all_feat_info_grps$cell_line <- factor(cell_25_all_feat_info_grps$cell_line, c("WT", "Cis"))
som_class_sel <- c(8, 3, 2, 14, 15, 24, 17, 23, 22, 20)

for (dx in som_class_sel){
    
  ## Bubble plots
    tDf <- cell_25_all_feat_info_grps[cell_25_all_feat_info_grps$som_class == dx,]
    tDf$Percent <- 100*tDf$nn/sum(tDf$nn)
    
    ggplot(tDf, aes(x = Dose, y = cell_line,  color = cell_line)) + 
      geom_point(aes(size = Percent), stroke = 0) + geom_text(size = 3, color= "black", aes(label = round(Percent, 2)), vjust = 2.5) + 
      ylab("Cell line") + xlab("Dose (nm)") + theme(axis.text.x = element_text(angle = 90)) + ggtitle(paste("Percent of cluster ", dx, "by cell line and dose")) + 
  scale_size_identity() 
    
    
    ggsave(paste0("/data/CEAT/ImageDatasets/CellPaint_WTCis_EpiCompoundDoseResponse/R_Results/SOM_Results_011223_CisOnly_2/SOM_Plots2/Percent_cells_inCluster", dx , ".png"),
           height = 5, width = 12)
    
    ## Make quick comp image
...
som_class_sel <- 20
for (idx in som_class_sel){

  

  tImDir <- dir(paste0("/data/CEAT/ImageDatasets/CellPaint_WTCis_EpiCompoundDoseResponse/R_Results/SOM_Results_011223_CisOnly_2/all_features/25_node/All_features_25_", idx), full.names = TRUE, )
  tImFn <- dir(paste0("/data/CEAT/ImageDatasets/CellPaint_WTCis_EpiCompoundDoseResponse/R_Results/SOM_Results_011223_CisOnly_2/all_features/25_node/All_features_25_", idx), full.names = FALSE, )

  #smp <- sample(length(tImDir), imN)
  if (length(tImFn) >= imN){smp <- sample(length(tImDir), 2*imN)} else {smp <- sample(length(tImDir), 2*imN, replace = TRUE)}
  ...
  ttIms <- (vector("list", length = (length(tIms)/2 - 1) * 2 + 1))
  ttIms2 <- ttIms
  ...
  for (i in 1:3) {
    ...
    ttIms2[[(i - 1) * 2 + 1]] <- tIms[[i + 3]]
    if (i < 3) {
      ttIms2[[(i - 1) * 2 + 2]] <- whiteIm
    }
  }
  
  im <- imappend(list(imappend(ttIms, axis = "x"), imappend(ttIms2, axis = "x")), axis = "y")
  save.image(im, paste0("/data/CEAT/ImageDatasets/CellPaint_WTCis_EpiCompoundDoseResponse/R_Results/SOM_Results_011223_CisOnly_2/SOM_Plots2/compSOM_Image_25Allfeat_", idx, ".png"))
}
```
