...
metCols <- colnames(P1_WT)[1:13]
datCols <- colnames(P1_WT)[14:length(colnames(P1_WT))]
...
save(WT_CisplatinUntreatedNrm, Cis_CisplatinUntreatedNrm, WT_Cis_CisplatinDoxUntreatedNrm, 
     WT_Cis_CorrelatedFeatures_IndNrm, WT_Cis_CorrelatedFeatures_CmbNrm, WT_CisplatinUntreated_nucLocs, Cis_CisplatinUntreated_nucLocs, 
     file = '/data/CEAT/ImageDatasets/CellPaint_WTCis_EpiCompoundDoseResponse/R_Results/WT_Cis_UT_Cis_Dox_Normed.rdata')
...
# Combined plot
# Scree plot
plot(WT_Cis_Cells_CmbNormPCA$sdev^2, type = "b", pch = 19, frame = FALSE, 
     xlab = "Principal Component", ylab = "Variance Explained",
     main = "Scree Plot", xlim = c(0,50))


plot(WT_Cis_Cells_SepNormPCA$sdev^2, type = "b", pch = 19, frame = FALSE, 
     xlab = "Principal Component", ylab = "Variance Explained",
     main = "Scree Plot", xlim = c(0,50))

```

## Perform SOM analysis on PCs
```{r}
# Use top 11 PCs
pcN <- 11

WT_Cis_Cmb_PCs <- cbind(WT_Cis_Cells_CmbNorm[,metCols], WT_Cis_Cells_CmbNormPCA$x[, 1:pcN])

set.seed(222)
#g <- somgrid(xdim = 5, ydim = 5, topo = "rectangular")
g <- somgrid(xdim = 6, ydim = 6, topo = "rectangular")
WT_Cis_SOM36_Cmb_PC <- som(as.matrix(WT_Cis_Cmb_PCs[, paste0("PC", 1:11)]), grid = g)

plot(WT_Cis_SOM36_Cmb_PC , type='codes', palette.name = rainbow)
plot(WT_Cis_SOM36_Cmb_PC , type = "mapping")

SOM36_Values_NrmCmb_PCs <- WT_Cis_SOM36_Cmb_PC$codes[[1]]
...
# consensusSOM36_Values_NrmCmb_PCsCanberra <-  ConsensusClusterPlus(t(SOM36_Values_NrmCmb_PCs),maxK=20,reps=50,pItem=0.8,pFeature=1, title="36 Node clustering",clusterAlg="hc",
#                                               seed=1262118388.71279, distance= "canberra", innerLinkage = "complete", finalLinkage = "complete")

pheatmap(SOM36_Values_NrmCmb_PCs, show_colnames =  FALSE, breaks = breaks, color = colors, clustering_distance_rows = "canberra", main = "canberra 15", cutree_rows = 15)
...
ptchMet <- WT_Cis_Cmb_PCs[, metCols]
ptchMet$FilenameCyto <- gsub("UV - DAPI","Green - dsRed" , ptchMet$FilenameDNA)


nucLocs2 <- dplyr::inner_join(ptchMet, nucLocs, by = c("FilenameDNA" = "FileName_DNA", "ObjectNumber_nuc" = "ObjectNumber_nuc"))

makeImagePatches(data.frame(ptchMet), (WT_Cis_SOM36_Cmb_PC$unit.classif), data.frame(nucLocs2),
                 "/data/CEAT/ImageDatasets/CellPaint_WTCis_EpiCompoundDoseResponse/R_Results/SOM_FromTopPCs/CmbNrm//",
                 prefix = "CmbNrmPCs", smpN = 20, setSeed = TRUE, seed = 47)
```
...
xxNrmCmbPCs <- cutree(hclust(dist(SOM36_Values_NrmCmb_PCs, "canberra")), k = 15)

## Plot the cluster composite images
compPlotsFromClusters("/data/CEAT/ImageDatasets/CellPaint_WTCis_EpiCompoundDoseResponse/R_Results/SOM_FromTopPCs/CmbNrm/",
                      "CmbNrmPCs_", xxNrmCmbPCs,
                      "/data/CEAT/ImageDatasets/CellPaint_WTCis_EpiCompoundDoseResponse/R_Results/SOM_FromTopPCs//CmbNrm//CompositeImages/")


 # pltCellsInClusters(WT_Cis_Cells_SepNorm, SOM36_Values_NrmInd, WT_Cis_SOM36_Ind_TpFt$unit.classif, metCols, "/data/CEAT/ImageDatasets/CellPaint_WTCis_EpiCompoundDoseResponse/R_Results/SOM_topPC_Parameters/CmbNrm/DoseDistributions/")

## Plot the distributions
 pltCellsInClusters(WT_Cis_Cells_CmbNorm, SOM36_Values_NrmCmb_PCs, WT_Cis_SOM36_Cmb_PC$unit.classif, metCols, "/data/CEAT/ImageDatasets/CellPaint_WTCis_EpiCompoundDoseResponse/R_Results/SOM_FromTopPCs/CmbNrm/DoseDistributions/")
 
```
## MST
```{r}

WT_Cis_SOM_MST <- makeMSTFromSOM(WT_Cis_SOM36_Cmb_PC)
meanClstSOMPar <- data.frame(matrix(ncol = ncol(WT_Cis_SOM36_Cmb_PC$codes[[1]]), nrow = 0))
colnames(meanClstSOMPar) <- colnames(WT_Cis_SOM36_Cmb_PC$codes[[1]])

for (dx in 1:length(unique(xxNrmCmbPCs))){
  
  tDf <- WT_Cis_SOM36_Cmb_PC$codes[[1]][names(xxNrmCmbPCs)[xxNrmCmbPCs == dx], , drop = FALSE]
  if (nrow(tDf) > 1){
  meanClstSOMPar[dx, ] <- colMeans(tDf)
  } else {
    
    meanClstSOMPar[dx, ] <- tDf
  }
}

WT_Cis_SOM_MST_TopPCs <- makeMSTFromSOM(meanClstSOMPar)
plot(WT_Cis_SOM_MST_TopPCs)
```
...
set.seed(43)
 ...
 smpN_Mofa <- 250
## Get the MOFA data sets for all cells ---------------------------------------------- #
...
MOFAOb_WT_Cis <- makeCpMOFAOb(WT_Cis_Cells_CmbNorm_MOFA )

## Get the MOFA data sets for all WT ---------------------------------------------- #
WT_Cells_CmbNorm_MOFA <- WT_Cis_CisplatinDoxUntreatedNrm[which(!grepl("Doxorubicin", WT_Cis_CisplatinDoxUntreatedNrm$Treatment)), ] %>% filter(cell_line == "WT") %>% group_by(Dose) %>% slice_sample(n = 2*smpN_Mofa)
MOFAOb_WT <- makeCpMOFAOb(WT_Cells_CmbNorm_MOFA )


## Get the MOFA data sets for all Cis ---------------------------------------------- #
Cis_Cells_CmbNorm_MOFA <- WT_Cis_CisplatinDoxUntreatedNrm[which(!grepl("Doxorubicin", WT_Cis_CisplatinDoxUntreatedNrm$Treatment)), ] %>% filter(cell_line == "Cis") %>% group_by(Dose) %>% slice_sample(n = 2*smpN_Mofa)
MOFAOb_Cis <- makeCpMOFAOb(Cis_Cells_CmbNorm_MOFA )

## Plot the MOFA object
plot_data_overview(MOFAOb_WT)
plot_data_overview(MOFAOb_Cis)
plot_data_overview(MOFAOb_WT_Cis)

## Make options

model_opts_WTCis <- get_default_model_options(MOFAOb_WT_Cis)

model_opts_WT <- get_default_model_options(MOFAOb_WT)

model_opts_Cis <- get_default_model_options(MOFAOb_Cis)


## Prepare MOFA object
MOFAOb_WT <- prepare_mofa(MOFAOb_WT,
  model_options = model_opts_WT
)

MOFAOb_Cis <- prepare_mofa(MOFAOb_Cis,
  model_options = model_opts_Cis
)

MOFAOb_WT_Cis <- prepare_mofa(MOFAOb_WT_Cis,
  model_options = model_opts_WTCis
)

## Write mofa object to rds
saveRDS(MOFAOb_WT, "/data/CEAT/ImageDatasets/CellPaint_WTCis_EpiCompoundDoseResponse/R_Results/MOFA/cellPaintMOFA_091123_WT_grp.rds")
saveRDS(MOFAOb_Cis, "/data/CEAT/ImageDatasets/CellPaint_WTCis_EpiCompoundDoseResponse/R_Results/MOFA/cellPaintMOFA_091123_Cis_grp.rds")
saveRDS(MOFAOb_WT_Cis, "/data/CEAT/ImageDatasets/CellPaint_WTCis_EpiCompoundDoseResponse/R_Results/MOFA/cellPaintMOFA_091123_WT_Cis_grp.rds")


```
## Get the MOFA objects
```{r}
MOFAOb_WT_Trained <- readRDS("/data/CEAT/ImageDatasets/CellPaint_WTCis_EpiCompoundDoseResponse/R_Results/MOFA/cellPaintMOFA_091123_trained_WT.rds")
MOFAOb_Cis_Trained <- readRDS("/data/CEAT/ImageDatasets/CellPaint_WTCis_EpiCompoundDoseResponse/R_Results/MOFA/cellPaintMOFA_091123_trained_Cis.rds")
MOFAOb_WT_Cis_Trained <- readRDS("/data/CEAT/ImageDatasets/CellPaint_WTCis_EpiCompoundDoseResponse/R_Results/MOFA/cellPaintMOFA_091123_trained_WT_Cis.rds")

plot_variance_explained(MOFAOb_WT_Trained, max_r2=20)
plot_variance_explained(MOFAOb_Cis_Trained, max_r2=20)
plot_variance_explained(MOFAOb_WT_Cis_Trained, max_r2=20)

MOFAOb_WT_Cis_Trained@cache$variance_explained$r2_per_factor[[1]] 
```


## Add the metadata
```{r}
WT_CmbsMOFA_meta <- cbind(data.frame(sample = paste0("Cell", 1:nrow(WT_Cells_CmbNorm_MOFA))),  WT_Cells_CmbNorm_MOFA[,metCols])
WT_CmbsMOFA_meta$Dose <- as.numeric(WT_CmbsMOFA_meta$Dose)
samples_metadata(MOFAOb_WT_Trained) <- data.table(WT_CmbsMOFA_meta)

Cis_CmbsMOFA_meta <- cbind(data.frame(sample = paste0("Cell", 1:nrow(Cis_Cells_CmbNorm_MOFA))),  Cis_Cells_CmbNorm_MOFA[,metCols])
Cis_CmbsMOFA_meta$Dose <- as.numeric(Cis_CmbsMOFA_meta$Dose)
samples_metadata(MOFAOb_Cis_Trained) <- data.table(Cis_CmbsMOFA_meta)
...
WT_CisCmbsMOFA_meta$Dose <- as.numeric(WT_CisCmbsMOFA_meta$Dose)
WT_CisCmbsMOFA_meta$Cell_Dose <- paste(WT_CisCmbsMOFA_meta$cell_line, WT_CisCmbsMOFA_meta$Dose, sep = "_")

WT_CisCmbsMOFA_meta$Cell_Dose <- factor(x = WT_CisCmbsMOFA_meta$Cell_Dose, levels = c(paste("WT",c(0, 500, 2000, 5000, 10000, 20000, 40000, 80000) , sep = "_"), paste("Cis",c(0, 500, 2000, 5000, 10000, 20000, 40000, 80000) , sep = "_")))

samples_metadata(MOFAOb_WT_Cis_Trained) <- data.table(WT_CisCmbsMOFA_meta)
```

## Correlate factors with covariates
```{r}
correlate_factors_with_covariates(MOFAOb_WT_Trained, 
  covariates = c("Dose", "plate"), 
  plot = "log_pval", max_log_pval = 10
  
)

correlate_factors_with_covariates(MOFAOb_Cis_Trained, 
  covariates = c("Dose", "plate"), 
  plot = "log_pval"
  
)

correlate_factors_with_covariates(MOFAOb_WT_Cis_Trained, 
  covariates = c("cell_line","Dose", "plate", "Cell_Dose"), 
  plot = "log_pval", groups = "group1"
  
)

correlate_factors_with_covariates(MOFAOb_WT_Cis_Trained, 
  covariates = c("cell_line","Dose", "plate", "Cell_Dose"), 
  plot = "r", groups = "group1"
  
)

plot_factor_cor(MOFAOb_WT_Cis_Trained)
```

## Plot facotrs together
```{r, fig.width=18}

plot_factors(MOFAOb_WT_Cis_Trained, 
  factors = c(1,2), 
  dot_size = 4.0, color_by = "Cell_Dose", shape_by = "cell_line"
)
...
## Factor plots
```{r, fig.width=8}
plot_factor(MOFAOb_WT_Cis_Trained, factors=1, group_by = "cell_line", color_by="Dose") +
  theme(
    axis.text.x = element_text(color="black", angle=40, vjust=1, hjust=1)
  )

plot_factor(MOFAOb_WT_Cis_Trained, factors=2, group_by = "cell_line", color_by="Dose") +
  theme(
    axis.text.x = element_text(color="black", angle=40, vjust=1, hjust=1)
  )
...
plot_factor(MOFAOb_WT_Cis_Trained, factors=1, color_by = "cell_line", group_by="Dose", ) +
  theme(
    axis.text.x = element_text(color="black", angle=40, vjust=1, hjust=1)
  )


plot_factor(MOFAOb_WT_Cis_Trained, factors=2, color_by = "cell_line", group_by="Dose", ) +
  theme(
    axis.text.x = element_text(color="black", angle=40, vjust=1, hjust=1)
  )
...
```
```{r, fig.width=8}
# plot_data_scatter(MOFAOb_WT_Cis_Trained, 
#   view = "AreaShape", 
#   factor = 1, 
#   features = 9,
#   color_by = "cell_line",
...
## Separate WT and Cis for loadings plots
```{r, fig.width=10}
library(gridExtra)
library(tidyr)
...
p <- plot_data_scatter_dj(MOFAOb_WT_Cis_Trained,  view = "DNA", factor = 2, n = 4,  x = "Dose", logX = TRUE)

p

pd <- plot_data_scatter_dj(MOFAOb_WT_Cis_Trained,  view = "DNA", factor = 2, n = 4,  x = "cell_line", logX = FALSE)

pd

pf2 <- plot_data_scatter_dj(MOFAOb_WT_Cis_Trained,  view = "DNA", factor = 2, n = 4,  logX = TRUE)

pf2

p <- plot_data_scatter_dj(MOFAOb_WT_Cis_Trained,  view = "DNA", factor = 2, n = 4,  x = "Dose", logX = TRUE)

pER <- plot_data_scatter_dj(MOFAOb_WT_Cis_Trained,  view = "ER", factor = 2, n = 9,   logX = FALSE)
pER

pER <- plot_data_scatter_dj(MOFAOb_WT_Cis_Trained,  view = "ER", factor = 2, n = 9, x = "Dose",  logX = FALSE)
pER
# pf2_Ds <- plot_data_scatter_dj(MOFAOb_WT_Cis_Trained,  view = "DNA", factor = 2, n = 4,  logX = TRUE, group = "Dose", colBrw = TRUE)
# 
# pf2_Ds


## Plot factor 1 against dose
fac1 = get_factors(MOFAOb_WT_Cis_Trained, factor = 1)$group1

metaDat <- MOFAOb_WT_Cis_Trained@samples_metadata

dfFac1 <- cbind(fac1, metaDat)

ggplot(dfFac1, aes(x = as.factor(Dose), y = fac1, fill = cell_line)) + 
  geom_violin() + geom_boxplot(width = 0.1) + stat_smooth(formula = y ~ x, aes_string(), 
        method = "lm", alpha = 0.4) + facet_wrap(~ cell_line, scales = "free_y")



## Plot factor 2 agaisnt dose


fac2 = get_factors(MOFAOb_WT_Cis_Trained, factor = 2)$group1

metaDat <- MOFAOb_WT_Cis_Trained@samples_metadata

dfFac2 <- cbind(fac2, metaDat)

ggplot(dfFac2, aes(x = as.factor(Dose), y = fac2, fill = cell_line)) + 
  geom_violin() + geom_boxplot(width = 0.1) + stat_smooth(formula = y ~ x, aes_string(), 
        method = "lm", alpha = 0.4) + facet_wrap(~ cell_line, scales = "free_y")


## Plot factor 5 agaisnt dose


fac5 = get_factors(MOFAOb_WT_Cis_Trained, factor = 5)$group1

metaDat <- MOFAOb_WT_Cis_Trained@samples_metadata

dfFac5 <- cbind(fac5, metaDat)

ggplot(dfFac5, aes(x = as.factor(Dose), y = fac5, fill = cell_line)) + 
  geom_violin() + geom_boxplot(width = 0.1) + stat_smooth(formula = y ~ x, aes_string(), 
        method = "lm", alpha = 0.4) + facet_wrap(~ cell_line, scales = "free_y")


## Plot factor 6 agaisnt dose


fac6 = get_factors(MOFAOb_WT_Cis_Trained, factor = 6)$group1

metaDat <- MOFAOb_WT_Cis_Trained@samples_metadata

dfFac6 <- cbind(fac6, metaDat)

ggplot(dfFac6, aes(x = as.factor(Dose), y = fac6, fill = cell_line)) + 
  geom_violin() + geom_boxplot(width = 0.1) + stat_smooth(formula = y ~ x, aes_string(), 
        method = "lm", alpha = 0.4) + facet_wrap(~ cell_line, scales = "free_y")
...
## Factor loadings etc
```{r}
plot_weights(MOFAOb_WT_Cis_Trained,
 view = "AreaShape",
 factor = 5,
